...
function unwrapGraphQLDef(typeDef) {
    ...
    while (graphql_1.isWrappingType(namedType)) {
        if (graphql_1.isListType(namedType)) {
            wrapping.unshift('List');
        }
        else if (graphql_1.isNonNullType(namedType)) {
...
function unwrapNexusDef(typeDef) {
    const wrapping = [];
    let namedType = typeDef;
    while (isNexusWrappingType(namedType) || graphql_1.isWrappingType(namedType) || nexusMeta_1.isNexusMetaType(namedType)) {
        if (nexusMeta_1.isNexusMetaType(namedType)) {
            namedType = nexusMeta_1.resolveNexusMetaType(namedType);
        }
        else if (graphql_1.isWrappingType(namedType)) {
            if (graphql_1.isListType(namedType)) {
                wrapping.unshift('List');
            }
            else if (graphql_1.isNonNullType(namedType)) {
...
function normalizeArgWrapping(argVal) {
    ...
    if (isNexusWrappingType(argVal)) {
        ...
        if (isNexusArgDef(namedType)) {
            ...
            return args_1.arg(Object.assign(Object.assign({}, config), { type: applyNexusWrapping(config.type, wrapping) }));
        }
        return args_1.arg({ type: applyNexusWrapping(namedType, wrapping) });
    }
...
function applyNexusWrapping(toWrap, wrapping) {
    ...
    wrapping.forEach((wrap) => {
        if (wrap === 'List') {
            finalType = list_1.list(finalType);
        }
        else if (wrap === 'NonNull') {
            finalType = nonNull_1.nonNull(finalType);
        }
        else if (wrap === 'Null') {
            finalType = nullable_1.nullable(finalType);
        }
        else {
            throw new utils_1.Unreachable(wrap);
        }
