...
function groupTypes(schema) {
    ...
    Object.keys(schemaTypeMap)
        ...
        else if (graphql_1.isScalarType(type) && !graphql_1.isSpecifiedScalarType(type) && !isUnknownType(type)) {
...
function runAbstractTypeRuntimeChecks(schema, features) {
    ...
    abstractTypes.forEach((type) => {
        const kind = graphql_1.isInterfaceType(type) ? 'Interface' : 'Union';
...
function getArgNamedType(argDef) {
    ...
    while (wrapping_1.isNexusWrappingType(finalValue) || graphql_1.isWrappingType(finalValue) || wrapping_1.isNexusArgDef(finalValue)) {
        if (wrapping_1.isNexusArgDef(finalValue)) {
            finalValue = finalValue.value.type;
        }
        else if (wrapping_1.isNexusWrappingType(finalValue)) {
            finalValue = finalValue.ofNexusType;
        }
        else if (graphql_1.isWrappingType(finalValue)) {
...
function getNexusNamedType(type) {
    ...
    let namedType = type;
    while (wrapping_1.isNexusWrappingType(namedType) || graphql_1.isWrappingType(namedType) || nexusMeta_1.isNexusMetaType(namedType)) {
        if (wrapping_1.isNexusWrappingType(namedType)) {
            namedType = namedType.ofNexusType;
        }
        if (graphql_1.isWrappingType(namedType)) {
            namedType = namedType.ofType;
        }
        if (nexusMeta_1.isNexusMetaType(namedType)) {
            namedType = nexusMeta_1.resolveNexusMetaType(namedType);
        }
    }
    return namedType;
}
